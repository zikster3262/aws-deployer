...
async function deployPrometheus(data) {
  ...
  const specString = await fsReadFileP(
    "./services/kubernetes/prometheus.yaml",
    "utf8"
  );
  ...
  logger.log.info("Prometheus created.");
  return created;
}

async function deletePrometheus(data) {
  const kubeConfig = await k8sConfig.createKubeconfig(data);
  const yaml = new YAML.Document();
  yaml.contents = kubeConfig;
  ...
  kc.loadFromString(yaml);
  const k8sApi = kc.makeApiClient(k8s.CoreV1Api);
  k8sApi.deleteNamespace("prometheus");
  logger.log.info("Delete Prometheus namespace.");
}
